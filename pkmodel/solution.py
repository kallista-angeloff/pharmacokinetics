...
def solution(model_sol, *args):
    ...
    Inputs
    ------
    model_sol (dict): The first set of model results. Should contain 2
                      keys ('dose', 'solution'). The dose values shows 
                      the amount of drug [units: ng] input with time, 
                      and the solution values show the amount of drug 
                      in the central (element 0) and peripheral 
                      (subsequent elements) compartments.

    *args: Optional arguments for more sets of model results. Should
           have same structure as model_sol.
